...
You are a senior software engineer. From the definitions above, ANSWER the following question by choosing only ONE alternative, using ONLY the NAME of the correct smell.

Question: In the test code below, which type of "Test Smell" can be identified? It is also possible to have no smell at all, in that case, answer "None".

{test_code}

Answer format: {{Smell Type}}

Directive: Provide only one alternative and DO NOT include any analysis about the code, only the smell type identified.
"""
