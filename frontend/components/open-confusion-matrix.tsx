...
import type { OpenProcessResult } from '@/lib/types'

interface OpenConfusionMatrixProps {
  results: OpenProcessResult[]
}
...
function buildOpenConfusionMatrix(subset: OpenProcessResult[], labels: string[]): ConfusionResult {
  const n = labels.length
  const matrix: number[][] = Array.from({ length: n }, () => Array(n).fill(0))

  subset.forEach(r => {
    // Skip API errors
    if (['TIMEOUT', 'API_ERROR', 'EMPTY_RESPONSE'].includes(r.normalizedResponse)) return

    const actual = r.testType
    const predicted = r.normalizedResponse
...
export function OpenConfusionMatrix({ results }: OpenConfusionMatrixProps) {
  ...
  const { confusionByModel, modelKeys } = useMemo(() => {
    ...
    results.forEach(r => {
      ...
      if (r.normalizedResponse && !['TIMEOUT', 'API_ERROR', 'EMPTY_RESPONSE'].includes(r.normalizedResponse)) {
        allLabels.add(r.normalizedResponse)
      }
    })
    const sortedLabels = Array.from(allLabels).sort()

    const byModelResults = new Map<string, OpenProcessResult[]>()
    results.forEach(r => {
    ...
    byModelResults.forEach((modelResults, key) => {
      confusionByModel.set(key, buildOpenConfusionMatrix(modelResults, sortedLabels))
    })
    confusionByModel.set('__all__', buildOpenConfusionMatrix(results, sortedLabels))
      ...
      <Card className="bg-card border-border">
        <CardContent className="pt-6">
          <p className="text-sm text-muted-foreground text-center">
            No valid data available for the confusion matrix. Run the Open Prompt test to generate data.
          </p>
        </CardContent>
      </Card>
    )
      ...
      <Card className="bg-card border-amber-500/20">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              <CardTitle className="text-base flex items-center gap-2">
                ...
                Confusion Matrix — Open Prompt
              </CardTitle>
              <CardDescription>
                Actual (rows) vs. Predicted (columns) — using normalized LLM responses
              </CardDescription>
            </div>
            <select
              value={selectedModel}
              onChange={(e) => setSelectedModel(e.target.value)}
              className="text-sm bg-secondary border border-border rounded-md px-3 py-1.5 text-foreground focus:outline-none focus:ring-1 focus:ring-amber-400"
            >
              {modelKeys.map(key => (
        ...
        <CardContent>
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Accuracy', value: cmData.macroAvg.accuracy },
              { label: 'Macro Precision', value: cmData.macroAvg.precision },
              { label: 'Macro Recall', value: cmData.macroAvg.recall },
              { label: 'Macro F1-Score', value: cmData.macroAvg.f1 },
            ].map(({ label, value }) => (
              <div key={label} className="text-center p-3 rounded-lg bg-secondary/30 border border-border/50">
                <p className="text-xs text-muted-foreground uppercase mb-1">{label}</p>
                <p className={cn('text-xl font-bold', colorByValue(value))}>{fmt(value)}</p>
              </div>
            ))}
          </div>
        </CardContent>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              <CardTitle className="text-base flex items-center gap-2">
                ...
                Per-Class Metrics — Open Prompt
              </CardTitle>
              <CardDescription>
                Precision, Recall, F1-Score and Accuracy for each test smell (based on normalized responses)
              </CardDescription>
            </div>
            <select
              value={selectedModel}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {/* Macro Average Row */}
                <tr className="border-t-2 border-amber-500/50 bg-amber-500/5 font-bold">
                  <td className="p-3 text-amber-400">Macro Average</td>
                  <td className="p-3 text-center text-muted-foreground">—</td>
                  <td className="p-3 text-center text-muted-foreground">—</td>
                  <td className="p-3 text-center text-muted-foreground">—</td>
                  <td className="p-3 text-center text-muted-foreground">—</td>
                  <td className="p-3 text-center text-amber-400">{fmt(cmData.macroAvg.precision)}</td>
                  <td className="p-3 text-center text-amber-400">{fmt(cmData.macroAvg.recall)}</td>
                  <td className="p-3 text-center text-amber-400">{fmt(cmData.macroAvg.f1)}</td>
                  <td className="p-3 text-center text-amber-400">{fmt(cmData.macroAvg.accuracy)}</td>
                </tr>
              </tbody>
            </table>
          </div>
